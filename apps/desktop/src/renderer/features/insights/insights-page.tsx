...
function getRhythmPlaceholderCellClassName(index: number): string {
  if (index % 9 === 0) {
...
function WeekdayRhythmPlaceholder({
  body,
  title,
}: {
  body: string;
    ...
    <Card className="border-dashed bg-muted/25">
      <CardHeader>
        <div>
          ...
          <CardDescription>
            Coming in a future data model update
          </CardDescription>
        </div>
      </CardHeader>
      <CardContent className="grid gap-4">
        <div className="grid grid-cols-7 gap-2 opacity-60">
          {Array.from({ length: 28 }, (_, index) => (
            <div
              className={cn(
                "h-8 rounded-md",
                getRhythmPlaceholderCellClassName(index)
              )}
              key={index}
            />
          ))}
    ...
    <div className="grid gap-6">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <WeekdayRhythmPlaceholder
          body={dashboard.weekdayRhythmPlaceholder.body}
          title={dashboard.weekdayRhythmPlaceholder.title}
        />
      </section>
    </div>
  );
